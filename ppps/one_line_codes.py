...
print(1 if int(num) == 0 else f'Factorial is {eval("*".join(str(i) for i in range(1, int(num) + 1)))}') # factorial of a number
print(f'sum is {sum(range(1, int(num) + 1))}') # sum of all numbers from 1 to num
print(f'Highest factor is {max(i for i in range(2, int(num)) if int(num) % i == 0)}' if int(num) > 1 else "No prime factor exists.") # highest factor
print('\n'.join([f"{num} x {i} = {num*i}" for i in range(1, 11)])) # multiplication table of a number
print("Its an Armstrong number" if sum(int(digit)**len(str(num)) for digit in str(num)) == num else "Not an Armstrong number") # checks for armstrong number
